...
2. В AWS EC2 нет прямого аналога параметру *core_fraction*, однако [T-серия](https://aws.amazon.com/ru/ec2/instance-types/) предоставляет подобную функциональность через механизм кредитов производительности: T-серия: Эти экземпляры обеспечивают базовый уровень производительности, подходящий для приложений с переменной или низкой интенсивностью использования процессора. Они накапливают кредиты производительности, которые могут быть использованы для всплесков активности, когда требуется более высокая производительность. Наиболее подходящим под характеристики является *t3.small* instanse type.</br>
